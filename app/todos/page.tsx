...
const TodosPage = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Failed to fetch todos from local storage:", error);
    } finally {
      setLoading(false);
    };
  }, [dispatch]);
      ...
      <div className="flex min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black px-4">
        <div className="flex flex-col gap-2 w-lg">
          ...
          <form className="flex gap-2">
            ...
            />
            <button
              className="flex h-12 w-full items-center justify-center gap-2 rounded-md bg-black text-white dark:bg-foreground dark:text-background px-5 transition-colors hover:bg-[#383838] dark:hover:bg-[#ccc] md:w-39.5 cursor-pointer"
          ...
          <div className="flex gap-2">
            <button
              className={`px-4 py-2 text-xs rounded-full ${filter === "all" ? "bg-foreground text-background" : "bg-gray-300 dark:bg-background text-black dark:text-white border border-white cursor-pointer"}`}
              onClick={() => setFilter("all")}
            >
              All
            </button>
            <button
              className={`px-4 py-2 text-xs rounded-full ${filter === "done" ? "bg-foreground text-background" : "bg-gray-300 dark:bg-background text-black dark:text-white border border-white cursor-pointer"}`}
              onClick={() => setFilter("done")}
            >
              Done
            </button>
            <button
              className={`px-4 py-2 text-xs rounded-full ${filter === "notDone" ? "bg-foreground text-background" : "bg-gray-300 dark:bg-background text-black dark:text-white border border-white cursor-pointer"}`}
          ...
          <ul className="flex flex-col gap-2 w-full h-96 overflow-y-auto">
            {filteredTodos.length > 0 ? (
              filteredTodos.map((todo) => (
                <li key={todo.id} className="w-full bg-gray-200 shadow-md dark:shadow-none dark:bg-gray-950 rounded-xl flex justify-between">
